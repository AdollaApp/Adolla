import { createFetch, ofetch, type $Fetch, type FetchOptions, type FetchRequest, type ResponseType } from "ofetch";
...
export function replaceExternalUrlWithInternal(url: string) {
  return url.replace(env.PUBLIC_EXTERNAL_API_BASEURL ?? '', env.PUBLIC_INTERNAL_API_BASEURL ?? '')
}

export const api = {
  fetch<T = any, R extends ResponseType = "json">(request: FetchRequest, options?: FetchOptions<R>) {
    return this.useFetch<T, R>(globalThis.fetch, request, options);
  },
  useFetch<T = any, R extends ResponseType = "json">(fetch: any, request: FetchRequest, options?: FetchOptions<R>) {
    const token = getAuth();
