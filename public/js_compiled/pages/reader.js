...
function updateScrollDebounce() {
  ...
  scrollDebounce = setTimeout(function () {
    if (!loaded) return; // Send POST request to update "reading" state
...
updatePages(); // Get page progress

function getPageProgress() {
  var _elementOffsets$;

  var pageCount = document.querySelectorAll(".pageImg").length; // Check if the reader is horizontal or not

  var isHorizontal = document.querySelector("[data-reader-direction]").getAttribute("data-reader-direction").startsWith("horizontal"); // Get offset for pages
...
function scrollReader(pageEl) {
  ...
  if (!readerIsHorizontal()) {
    window.scrollBy(0, -topOffset); // Deal with iOS padding
  }
...
function nextPage() {
  ...
  if (pageEl) scrollReader(pageEl); // Go to next chapter if it's on the last page
...
function previousPage() {
  ...
  if (pageEl) scrollReader(pageEl); // Go to previous chapter if it's on page 0
...
document.querySelectorAll(".manga-reader").forEach(function (el) {
  el.addEventListener("click", function (evt) {
    ...
    }).join(" ").trim(); // If a button was pressed or the layout is open, do nothing
    ...
    var relativeX = clientX - containerX; // Evaluate which section was clicked
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 11:
            ...
            setLoadingText("");
            updatePages(); // Update loading section in DOM

            document.querySelector(".manga-reader").classList.add("loaded"); // Check whether to scroll to page yet, or not
...
function doImages() {
  ...
  var _loop = function _loop() {
    ...
    img.style.minHeight = "30vh"; // Set source
  ...
  btn.addEventListener("click", function () {
    ...
    setTimeout(function () {
      var f = function f(evt) {
        ...
        clearInterval(cameraInterval); // Get image URLs for every image that was on screen
...
function updateDoublePages() {
  ...
  var wrapper = document.querySelector(".pages"); // Get all images
