	...
	helpers: {
		...
		getProgressString(manga: StoredData) {
			if(manga.progress && manga.success) {
				...
				const curChapter = manga.data.chapters.find(c => c.hrefString === manga.progress.chapterId);
		...
		genLink2(provider = "mangasee", slug: string, hrefString: string | null = null, chapter = -1) {
		...
		getChapterName(progress: Progress, manga: StoredData) {
			...
			const current = manga.data.chapters.find(v => v.hrefString === progress.chapterId);
