...
export class MangaseeClass extends Scraper {
	 ...
	 * @param season 
	 */
	public scrape(slug: string, chapterId: string | number | null = null): Promise<ScraperResponse> {
		return new Promise(async resolve => {
			...
			setTimeout(() => {
				if(!isResolved) {
					isResolved = true;
					console.error("Timed out", slug, chapterId);
					resolve(error(504, "Custom time out"));
				}
			}, 10e3);
			...
			if(chapterId && typeof chapterId === "string") {
				...
				if(!chapterMatch && !isResolved) {
					isResolved = true;
					resolve(error(403, "Invalid season chapter string"));
					return;
				}
			...
			try {
				...
				if(!pageRes.ok || pageRes.url.endsWith("undefined") || html.includes(`<title>404 Page Not Found</title>`)) {
					if(!isResolved) {
						...
						isResolved = true;
						resolve(error(pageRes.status, html));
						return;
					}
				}
